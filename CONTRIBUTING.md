...
1. **中文标题文章**：
   - 文件名使用中文：`2024-10-29-房地产系列.md`
   - 或使用拼音：`2024-10-29-fangdichan-xilie.md`

2. **英文标题文章**：
   - 文件名使用英文：`2024-10-29-react-hooks-guide.md`
...
1. **正确的图片路径**：
   - 从语雀导入的图片会自动进行代理处理
   - 本地图片推荐使用项目相对路径
   - 推荐结构：`/assets/images/posts/分类/YYYY-MM-DD-标题/图片名.jpg`
   - 例如：`/assets/images/posts/tech/2023-06-15-hello-world/example.jpg`
...
3. **图片格式**：
   - 推荐使用WebP格式（更小的体积，更好的质量）
   - 也支持JPG、PNG、GIF格式
   - 考虑使用图片压缩工具如TinyPNG处理图片

4. **图片命名规范**：
   - 使用有意义的名称，避免随机字符串
   - 可以使用中文命名，但建议使用英文或拼音以避免URL编码问题
   - 示例：`banner.jpg`、`code-example.png`、`workflow-diagram.webp`
...
4. **图片优化**：
   - 所有图片添加alt属性
   - 图片名称包含关键词
   - 示例：`![React组件生命周期图](/assets/images/react-lifecycle.jpg)`

5. **内部链接**：
   - 适当添加站内其他相关文章的链接
   - 使用描述性锚文本
   - 示例：`查看更多关于[React Hooks的使用方法](/posts/tech/2023-05-10-react-hooks.md)`
