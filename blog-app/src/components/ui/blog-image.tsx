...
export function BlogImage({ src, alt, caption, className = '', targetId }: BlogImageProps) {
  ...
  useEffect(() => {
    ...
    const processImageUrl = (url: string): string => {
      ...
      if (url.includes('cdn.nlark.com') || url.includes('yuque.com')) {
        ...
        console.log(`[BlogImage] 代理URL: /api/proxy?url=${encodedUrl}`);
        
        // 添加时间戳参数防止缓存
        return `/api/proxy?url=${encodedUrl}&t=${Date.now()}`;
      }
  ...
  const handleError = (e: React.SyntheticEvent<HTMLImageElement, Event>) => {
    ...
    if (tryCount < 3) {
      ...
      // 根据URL类型选择不同的重试策略
      if (imageSrc.includes('docs/images/')) {
        ...
        // 默认行为：尝试使用图片代理服务
        const proxyUrls = [
          `/api/proxy?url=${encodeURIComponent(imageSrc)}`,
