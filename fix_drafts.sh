...
# 脚本用于修复drafts目录下所有Markdown文件的Front Matter和图片路径
# 使文件符合CONTRIBUTING.md的要求

# 处理drafts目录下的所有Markdown文件
find /Users/dada/Documents/dada_bolg/docs/drafts -name "*.md" | while read file; do
  echo "处理文件: $file"
  ...
  if ! grep -q "^---" "$file"; then
    ...
    echo "categories: [草稿]" >> "$temp_file"
    echo "tags: [草稿]" >> "$temp_file"
    echo "published: false" >> "$temp_file"
    echo "---" >> "$temp_file"
    echo "" >> "$temp_file"
    cat "$file" >> "$temp_file"
    mv "$temp_file" "$file"
    ...
    awk '
    BEGIN { in_front_matter = 0; has_title = 0; has_date = 0; has_categories = 0; has_published = 0; }
    /^---/ {
      if (in_front_matter == 0) {
        ...
        if (!has_categories) print "categories: [草稿]";
        if (!has_published) print "published: false";
        print "---";
        next;
      }
    }
    ...
    in_front_matter == 1 && /^published:/ { has_published = 1; }
    { print; }
    ' "$file" > "${file}.tmp" && mv "${file}.tmp" "$file"
  fi
done

echo "所有草稿Markdown文件处理完成!" 