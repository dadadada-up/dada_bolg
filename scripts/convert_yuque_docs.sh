...
# 设置日志文件
LOG_FILE="convert_yuque_docs_log.txt"
echo "开始转换语雀文档 $(date)" > $LOG_FILE
...
download_and_update_images() {
  ...
  # 提取图片链接
  local img_urls=$(grep -o '!\[.*\](https://cdn.nlark.com/yuque/[^)]*' "$file" | sed 's/!\[.*\](\(.*\)/\1/')
...
categorize_document() {
  ...
  elif [[ "$filename" == *"Cursor"* || "$filename" == *"技术"* || "$filename" == *"GitHub"* || "$filename" == *"Docker"* ]]; then
    category="tech-tools"
  elif [[ "$filename" == *"金融"* || "$filename" == *"投资"* || "$filename" == *"基金"* || "$filename" == *"房地产"* ]]; then
    category="finance"
  ...
  elif [[ "$filename" == *"读后感"* || "$filename" == *"读书"* ]]; then
    category="reading"
  else
    # 如果文件名无法判断，则尝试通过内容判断
...
process_file() {
  ...
  # 下载图片并更新链接
  download_and_update_images "$file" "$category"
