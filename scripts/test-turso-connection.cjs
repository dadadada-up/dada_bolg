/**
 * Turso数据库连接测试脚本 (CommonJS版本)
 * 
 * 用法:
 *   node scripts/test-turso-connection-cjs.js
 */
...
async function testTursoConnection() {
  ...
  if (!TURSO_URL) {
    ...
    log(`当前环境变量: ${JSON.stringify(process.env)}`, 'debug');
  ...
  try {
    ...
    console.log('查询结果:', JSON.stringify(testResult));
