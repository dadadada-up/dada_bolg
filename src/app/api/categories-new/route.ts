...
export async function GET() {
  try {
    ...
    try {
      ...
      console.error('[API] 数据库查询失败，使用备用分类数据:', dbError);
    ...
    // 最终备用：返回备用数据
    console.log('[API] 使用备用分类数据作为最终备份');
    return Response.json(fallbackCategories);
  }
}
  ...
  try {
    ...
    try {
      ...
      return Response.json(
        { error: '数据库连接失败，请稍后再试', details: dbError instanceof Error ? dbError.message : String(dbError), fallback: true },
        { status: 503 }
      );
    }
  } catch (error) {
