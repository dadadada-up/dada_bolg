...
import { getDatabaseType, getDatabaseUrl, getDatabaseAuthToken, isVercelEnv } from './env-config';
...
if (isVercelEnv) {
  try {
    ...
    throw error; // 直接抛出错误,不使用备用数据
  }
} else {
  console.log('[数据库] 本地环境不使用Turso');
}
